...
void Shader::SetVec3(char *name, glm::vec3 v)
{
	int location = GetLocation(name);
	glUniform3f(location, v.x, v.y, v.z);
}

void Shader::SetMat4(char *name, glm::mat4 m)
{
	int location = GetLocation(name);
	glUniformMatrix4fv(location, 1, GL_FALSE, glm::value_ptr(m));
}

void Shader::SetVec4(char *name, float x, float y, float z, float s)
{
	int location = GetLocation(name);
	glUniform4f(location, x, y, z, s);
}


void Shader::SetVec4(char *name, glm::vec4 v)
{
	int location = GetLocation(name);
